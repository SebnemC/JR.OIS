    ...
    public function kaydetAction()
    {
      ...
      if($id){
        ...
        if($tbl->update($post,$where)){
          $this->session->bilgiMesaji="Güncelleme İşlemi Gerçekleştirildi.";
        }
        else
          $this->session->hataMesaji="Güncelleme olamadi :(";
      }
      else{
        $id=$tbl->insert($post);
        if($id)
          $this->session->bilgiMesaji="Kaydetme İşlemi Gerçekleştirildi.";
        else
